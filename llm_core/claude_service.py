...
class ClaudeClient:
    def __init__(self):
        ...
        self._default_model = os.getenv("ANTHROPIC_DEFAULT_MODEL", "claude-3-5-sonnet-20241022")
        self._default_timeout = int(os.getenv("ANTHROPIC_TIMEOUT", "60"))
        self._base_url = "https://api.anthropic.com/v1/messages"
        
        if not self._api_key:
            raise RuntimeError("ANTHROPIC_API_KEY no está configurada.")
        logger.info("ClaudeClient inicializado con modelo: %s", self._default_model)
    ...
    def _post(self, body: dict, timeout: int) -> dict:
        """Realiza petición POST a la API de Claude."""
        headers = {
    ...
    def ask_claude_image(self, prompt: str, image_path: str | Path, 
                        model: Optional[str] = None, timeout: Optional[int] = None) -> str:
        """Consulta que incluye imagen a Claude."""
...
def ask_claude_image(prompt: str, image_path: str | Path, 
