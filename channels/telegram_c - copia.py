...
async def process_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        resultado = pip_processor.process_image(image_stream, session_id=session_id)
        if resultado and resultado.get("success"):
            await update.message.reply_text("✅ Prescripción procesada exitosamente.")
        else:
            await update.message.reply_text("⚠️ No pude procesar la imagen. ¿Podrías intentar con otra foto?")
    except Exception as e:
